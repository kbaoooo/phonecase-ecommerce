    ...
    return (
        ...
        >
            {reviews?.concat(reviews).map((imgSrc, index) => (
                <Review key={index} className={reviewClassName?.(index % reviews.length)} imgSrc={imgSrc}/>
            ))}
        </div>
    )
}
...
function Review({imgSrc, className, ...props}: ReviewProps) {
    ...
    return (
        <div {...props} className={cn('animate-fade-in rounded-[2.25rem] bg-white opacity-0 p-6 shadow-xl shadow-slate-900/5', className)} style={{animationDelay}}>
            <Phone imgSrc={imgSrc} />
        </div>
    )
}
